"""Produce source-linked spec and manifest-derived tables."""
import hashlib,json,pathlib
from roms import R,manifest
def main():
 d=R/'docs';d.mkdir(exist_ok=True);m=manifest();commit=m['mame_commit'];url=f'https://github.com/mamedev/mame/blob/{commit}/src/mame/sanritsu/drmicro.cpp'
 src=(R/'reference/mame/src/mame/sanritsu/drmicro.cpp').read_text().splitlines()
 def ref(start,end):return f'[{start}–{end}]({url}#L{start}-L{end})'
 def section(token,stop):
  a=next(i+1 for i,s in enumerate(src) if token in s);b=next(i+1 for i,s in enumerate(src[a:],a) if stop in s);return ref(a,b)
 spec=f'''# Dr. Micro hardware specification

Functional baseline: MAME commit `{commit}`. The complete driver is retained in `reference/mame/src/mame/sanritsu/drmicro.cpp`. Source line numbers below refer to that immutable revision. This is an emulation-derived specification, not a PCB timing measurement.

## CPU and address maps

One Z80, normal mode, 18.432 MHz / 6 = 3.072 MHz (source marks this clock with a question). Three SN76496 share this nominal input rate. MSM5205 input 384 kHz, S64_4B = 6 kHz. Refresh is 60 Hz, 256×256 logical pixels, visible x=0..255, y=16..239. ROT270 at presentation. Source: {section('void drmicro_state::drmicro(machine_config','ROM_START')}.
...
MAME layouts: {section('static const gfx_layout spritelayout4','Machine driver')}.
...
Control port 04 bit 0 gates one NMI per VBLANK, bit 1 is the game flip latch; bits 2/3 unknown. Reset sets NMI enable, flip and sample address to zero. No level IRQ is added. Source: {section('INTERRUPT_GEN_MEMBER(drmicro_state::interrupt)','Address maps')}.
...
All defined controls are active high. P1 bits 0..6 = up,right,down,left,action,coin,service. P2 bits 0..6 = up,right,down,left,action,start1,start2. Bit 7 unknown/zero. Both sticks are four-way. Full original input block: {section('static INPUT_PORTS_START','Graphics definitions')}.

The following options/defaults are generated from the pinned source; hardware SW locations are inverted (!). Defaults: DSW1=0x{m['dip_defaults'][0]:02x}, DSW2=0x{m['dip_defaults'][1]:02x}.

| Bank | Mask | Name | Default | Encodings |
|---|---|---|---|---|
'''
 for dip in m['dips']:
  spec+=f"| {dip['bank']+1} | {dip['mask']:02x} | {dip['name']} | {dip['default']:02x} | "+'; '.join(f"{o['value']:02x}={o['name']}" for o in dip['options'])+' |\n'
 spec+='''
## Uncertainties
...
'''
 (d/'hardware_spec.md').write_text(spec,encoding='utf-8')
 layout='# Canonical ROM payload\n\nGenerated from the pinned MAME physical ROM definitions. No header, padding or trailer. Bytes follow region/file order. CPU allocation is 64 KiB but only its populated 48 KiB is packed. The total is '+str(m['length'])+' bytes. MRA, packer, loader constants and simulation use `reference/rom_manifest.json`.\n\n| Region | Packed offset | Populated | MAME allocation |\n|---|---:|---:|---:|\n'
 for r in m['regions']:layout+=f"| {r['name']} | 0x{r['offset']:05x} | 0x{r['length']:x} | 0x{r['allocated']:x} |\n"
 ...
 (d/'rom_layout.md').write_text(layout)
